/**
 * Generated by C-Next Transpiler from: J1939Encode.cnx
 * A safer C for embedded systems
 */

#include "Display/J1939Encode.hpp"
...
uint16_t J1939Encode_encode(float value, float resolution, float offset) {
    ...
    return ((scaled) > ((float)UINT16_MAX) ? static_cast<uint16_t>(UINT16_MAX) : (scaled) < 0.0f ? 0 : static_cast<uint16_t>((scaled)));
}
...
uint8_t J1939Encode_temp8bit(float temperatureC) {
    ...
    return ((offset) > ((float)UINT8_MAX) ? static_cast<uint8_t>(UINT8_MAX) : (offset) < 0.0f ? 0 : static_cast<uint8_t>((offset)));
}

uint8_t J1939Encode_humidity(float humidityPercent) {
    float scaled = humidityPercent / 0.4;
    return ((scaled) > ((float)UINT8_MAX) ? static_cast<uint8_t>(UINT8_MAX) : (scaled) < 0.0f ? 0 : static_cast<uint8_t>((scaled)));
}
